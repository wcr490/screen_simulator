...
int mem_malloc(unsigned int msize){
	...
	// Used as a container for collecting the member in struct and memcpy to the target memory block at the actual address
	mem_block tmp_blk;
	if(msize == 0) return 0;
	if(sum){
		// pointer in the memory pool currently
