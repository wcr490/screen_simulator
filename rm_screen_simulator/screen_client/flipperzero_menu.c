...
#define NUM_ITEMS 8        // number of items in the list and also the number of screenshots and screenshots with QR codes (other screens)
...
title_t menu_items[NUM_ITEMS] = { // array with item names
    {"3D Cube", 7},
    {"Battery", 7},
    {"Dashboard", 9},
    {"Fireworks", 9},
...
void loop()
{
    ...
    do
    {
        ...
        { // MENU SCREEN
            ...
            draw_char_arr_buffer(sock, bitmap_item_sel_outline, 0, 22, 128, 21);
            ...
            draw_str8x14_buffer(sock, menu_items[item_sel_previous].name, 25, 2, menu_items[item_sel_previous].len);
            draw_char_arr_buffer(sock, bitmap_icons[item_sel_previous], 4, 2, 16, 16);
            ...
            draw_str8x14_buffer(sock, menu_items[item_selected].name, 25, 2 + 20 + 2, menu_items[item_selected].len);
            draw_char_arr_buffer(sock, bitmap_icons[item_selected], 4, 24, 16, 16);
            ...
            draw_str8x14_buffer(sock, menu_items[item_sel_next].name, 25, 2 + 20 + 20 + 2 + 2, menu_items[item_sel_next].len);
            draw_char_arr_buffer(sock, bitmap_icons[item_sel_next], 4, 46, 16, 16);

            // draw scrollbar background
            // u8g.drawBitmapP(128 - 8, 0, 8 / 8, 64, bitmap_scrollbar_background);

            draw_char_arr_buffer(sock, bitmap_scrollbar_background, 128 - 8, 0, 8, 64);

            // draw scrollbar handle
            // u8g.drawBox(125, 64 / NUM_ITEMS * item_selected, 3, 64 / NUM_ITEMS);

            draw_rectangle_buffer(sock, 125, 64 / NUM_ITEMS * item_selected, 64 / NUM_ITEMS, 3);

            // draw upir logo
            // u8g.drawBitmapP(128 - 16 - 4, 64 - 4, 16 / 8, 4, upir_logo);

            draw_char_arr_buffer(sock, upir_logo, 128 - 16 - 4, 64 - 4, 16, 4);
            update_buffer(sock);
        }
        else if (current_screen == 1)
        { // SCREENSHOTS SCREEN
    ...
    } while (1); // required for page drawing mode with u8g library
}