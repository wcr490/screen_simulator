...
void panel_x_set_cb(void *obj, short val)
{
	...
	panel->r = lv_map(panel->x, 0, 100, 0, 6);
	// panel = &panel2;
	/*
	panel->x = 100 - val;
	panel->w = lv_map(panel->x, 0, 100, 2, 30);
...
{
	egg_penel_init(&panel1, 65, 2, 61, 28, 5, 1);
}
...
void draw_test_1_init(void *p)
{
	...
	egg_switch_init(&btn, 30, 0, 80, 15, 0, 0);
	// get_sin_tab1(128,32);
}
